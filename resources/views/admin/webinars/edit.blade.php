...
<div class="max-w-12xl mx-auto p-6 bg-white rounded-xl shadow-lg">
    <h2 class="text-3xl font-bold mb-6 flex items-center gap-2 text-gray-800">
        <i class="fas fa-video text-blue-600"></i> Edit Webinar
    </h2>

    <form action="{{ route('admin.webinars.update', $webinar) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <div class="flex items-center border border-gray-300 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-blue-300">
                ...
                <input type="text" name="title" value="{{ old('title', $webinar->title) }}" placeholder="Enter webinar title"
        ...
        <div>
            ...
            <div class="flex items-center border border-gray-300 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-blue-300">
                ...
                <input type="text" name="author" value="{{ old('author', $webinar->author) }}"
        ...
        <div>
            ...
            <div class="flex items-start border border-gray-300 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-blue-300">
                ...
                <textarea name="excerpt" rows="3" placeholder="Short summary for preview..."
                          class="w-full p-2 outline-none focus:ring-0">{{ old('excerpt', $webinar->excerpt) }}</textarea>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="border border-gray-300 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-blue-300">
                <textarea id="content-editor" name="content" placeholder="Describe the webinar in detail..." required>{{ old('content', $webinar->content) }}</textarea>
            </div>
        </div>
        ...
        <div>
            ...
            @if($webinar->featured_image)
                <p class="text-sm text-gray-500 mt-2">Current: <img src="{{ asset('storage/'.$webinar->featured_image) }}" alt="" class="h-16 inline-block rounded"></p>
            @endif
        </div>
